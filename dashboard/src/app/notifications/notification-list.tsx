...
export function NotificationList({
  ...
  const { data: configs, mutate } = useSWR<NotificationConfig[]>("/api/notifications", fetcher, {
    fallbackData: initialConfigs,
  });
  const { data: rules } = useSWR<Rule[]>("/api/rules", fetcher, {
  ...
  const ruleName = (id: string) => ruleById.get(id) ?? id;
