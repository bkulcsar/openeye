...
export default function ZonesPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/tripwires").then((r) => r.json()),
  ...
  const handleDeleteTripwire = async () => {
    ...
    if (res.ok) {
      setTripwires(tripwires.filter((t) => t.id !== deleteTripwire.id));
